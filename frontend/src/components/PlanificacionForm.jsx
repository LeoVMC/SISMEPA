...
export default function PlanificacionForm() {
  ...
  const submit = async (e) => {
    ...
    const asignatura = form.asignatura.value
    if (!file) {
    ...
    xhr.open('POST', 'http://localhost:8000/api/planificaciones/')
  ...
  return (
    <form onSubmit={submit} className="p-4 bg-white rounded shadow mt-2">
      <h4 className="font-medium">Subir Planificación (Docente)</h4>
      <div className="mt-2">
        <input name="asignatura" placeholder="asignatura id" className="border p-1 mb-2" />
